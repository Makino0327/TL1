...
def get_level_stage_collection_name(stage):

    # ステージ種類から対応するコレクション名を返す
    if stage == "BOSS":
        return "BossStage"
...
def ensure_level_stage_collection(scene, stage):

    # MainStage/BossStageコレクションを必要に応じて作る
    collection_name = get_level_stage_collection_name(stage)
...
def ensure_level_stage_collections(scene):

    # 通常ステージとボスステージの置き場所を両方用意する
    ensure_level_stage_collection(scene, "MAIN")
    ensure_level_stage_collection(scene, "BOSS")


def move_object_to_level_stage(object, scene, stage):

    # オブジェクトを現在編集中のステージコレクションへ入れる
    if object is None:
        return

    target_collection = ensure_level_stage_collection(scene, stage)
    if target_collection.objects.get(object.name) is None:
        target_collection.objects.link(object)

    # MainStage/BossStageのどちらかだけへ所属させて、表示と出力が混ざらないようにする
    for collection in list(object.users_collection):
        if collection != target_collection:
            collection.objects.unlink(object)

    object["level_stage"] = stage

def move_object_and_children_to_level_stage(object, scene, stage):

    # 親子構造をまとめて同じステージへ移動する
    move_object_to_level_stage(object, scene, stage)

    for child in object.children:
        move_object_and_children_to_level_stage(child, scene, stage)


def get_object_level_stage(object):

    # オブジェクトが所属しているステージを返す
    if object is None:
        return "MAIN"

    # コレクション所属を最優先にする
    for stage in ["MAIN", "BOSS"]:
        collection = bpy.data.collections.get(get_level_stage_collection_name(stage))
        if collection is not None and collection.objects.get(object.name) is not None:
            return stage

    # どちらのコレクションにも入っていない既存オブジェクトは通常ステージ扱いにする
    return "MAIN"

def normalize_existing_level_stage_objects(scene):

    # 旧データや未分類オブジェクトをMainStageへ集める
    main_collection = ensure_level_stage_collection(scene, "MAIN")
    boss_collection = ensure_level_stage_collection(scene, "BOSS")

    for object in scene.objects:
        # Main/Bossのどちらかに既に入っているものはそのままにする
        is_in_main = main_collection.objects.get(object.name) is not None
        is_in_boss = boss_collection.objects.get(object.name) is not None
        if is_in_main or is_in_boss:
            continue

        # 編集対象になりうる既存オブジェクトだけMainStageへ移す
        object_kind = get_object_kind(object)
        if object_kind not in {"enemy", "wall", "floor", "player", "navmesh", "boss_teleport"}:
            continue

        if main_collection.objects.get(object.name) is None:
            main_collection.objects.link(object)

        object["level_stage"] = "MAIN"


def should_export_object_for_stage(object, stage):

    # ALL指定なら従来通りすべて出力する
    if stage == "ALL":
        return True

    return get_object_level_stage(object) == stage


def sync_level_stage_visibility(scene):

    # Editで選んだステージだけを表示する
    ensure_level_stage_collections(scene)
    normalize_existing_level_stage_objects(scene)
    edit_stage = scene.myaddon_edit_level_stage
    ...
    for stage in ["MAIN", "BOSS"]:
        ...
        is_visible = (stage == edit_stage)
        collection.hide_viewport = not is_visible
        collection.hide_render = not is_visible

    for object in scene.objects:
        object_stage = get_object_level_stage(object)
        is_visible = (object_stage == edit_stage)

        # NavMeshはEdit側でもShow NavMeshがOFFなら隠す
        if is_nav_mesh_object(object) and not scene.myaddon_show_nav_mesh:
            is_visible = False

        object.hide_viewport = not is_visible
        object.hide_render = not is_visible
        object.hide_set(not is_visible)

        if not is_visible and object.select_get():
            object.select_set(False)

def on_edit_level_stage_changed(self, context):

    # UIで編集ステージを切り替えたら表示も切り替える
...
def is_boss_teleport_object(object):

    # ボスステージへ移動する入口かどうかを判定する
    if object is None:
        return False

    if object.get("object_kind", "") == "boss_teleport":
        return True

    return object.name.startswith("BossTeleport_") or object.name == "BossTeleport"
...
def get_object_kind(object):
    ...
    if is_boss_teleport_object(object):
...
def refresh_object_kind_tag(object):
    ...
    elif object_kind == "boss_teleport":
...
def sync_object_selection_filter(scene):
    ...
    for object in scene.objects:
        ...
        is_editable_object = object_kind in {"enemy", "wall", "floor", "player", "navmesh", "boss_teleport"}
...
def add_enemy_waypoint(context, enemy_object):
    ...
    # Waypointは親の敵と同じステージへ入れる
    move_object_to_level_stage(waypoint_object, context.scene, get_object_level_stage(enemy_object))
...
def create_enemy_from_model(context, enemy_type_name):
    ...
    # 現在編集中のステージへ敵本体を入れる
    move_object_to_level_stage(enemy_object, context.scene, context.scene.myaddon_edit_level_stage)
...
class MYADDON_OT_add_player_spawn(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # 現在のEditステージにPlayerが既にある場合だけ再利用する
        current_stage = context.scene.myaddon_edit_level_stage
        existing_object = None
        for scene_object in context.scene.objects:
            if get_object_kind(scene_object) != "player":
                continue
            if get_object_level_stage(scene_object) != current_stage:
                continue
            existing_object = scene_object
            break
        ...
        if existing_object is not None:
            ...
            sync_level_stage_visibility(context.scene)
            sync_object_selection_filter(context.scene)
            self.report({'INFO'}, "Player already exists in this stage")
            return {'FINISHED'}
        ...
        if player_object is None:
            ...
            return {'CANCELLED'}
        rename_imported_object(player_object, "Player" if current_stage == "MAIN" else "Player_Boss")

        # Store metadata used by the game side.
        player_object["file_name"] = "player/player.obj"
        player_object["object_kind"] = "player"
        # 現在編集中のステージへプレイヤー初期位置を入れる
        move_object_to_level_stage(player_object, context.scene, context.scene.myaddon_edit_level_stage)
...
def has_stage_object_kind(scene, stage, object_kind):

    # 指定ステージに指定種類のオブジェクトがあるか調べる
    for object in scene.objects:
        if get_object_kind(object) != object_kind:
            continue
        if get_object_level_stage(object) != stage:
            continue
        return True
...
def create_stage_floor(scene, stage):

    # 指定ステージに出力用の床を作る
    floor_index = find_next_numbered_name(scene, "floor_")
    floor_name = f"floor_{floor_index:02d}"

    bpy.ops.mesh.primitive_cube_add(location=(0.0, 0.0, -1.0))
    floor_object = bpy.context.active_object
    floor_object.name = floor_name
    floor_object.scale = (25.0, 25.0, 1.0)
    floor_object["file_name"] = "cube.obj"
    floor_object["collider"] = "BOX"
    floor_object["collider_center"] = mathutils.Vector((0, 0, 0))
    floor_object["collider_size"] = mathutils.Vector((2, 2, 2))
    floor_object["object_kind"] = "floor"
    move_object_to_level_stage(floor_object, scene, stage)
    apply_editor_visuals(floor_object)

    return floor_object


def ensure_boss_stage_basics(scene):

    # bossStage.jsonが空にならないよう、最低限の床とPlayerをBoss側へ用意する
    ensure_level_stage_collections(scene)

    if not has_stage_object_kind(scene, "BOSS", "floor"):
        create_stage_floor(scene, "BOSS")

    if not has_stage_object_kind(scene, "BOSS", "player"):
        previous_stage = scene.myaddon_edit_level_stage
        previous_cursor = scene.cursor.location.copy()
        scene.myaddon_edit_level_stage = "BOSS"
        scene.cursor.location = mathutils.Vector((0.0, 0.0, 0.0))
        bpy.ops.myaddon.add_player_spawn()
        scene.cursor.location = previous_cursor
        scene.myaddon_edit_level_stage = previous_stage

    sync_level_stage_visibility(scene)
    sync_object_selection_filter(scene)
...
class MYADDON_OT_generate_nav_mesh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # 現在編集中のステージへNavMeshを入れる
        move_object_to_level_stage(nav_mesh_object, context.scene, context.scene.myaddon_edit_level_stage)
...
class MYADDON_OT_add_wall(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # 現在編集中のステージへ壁を入れる
        move_object_to_level_stage(wall_object, context.scene, context.scene.myaddon_edit_level_stage)
...
class MYADDON_OT_add_floor(bpy.types.Operator):

    bl_idname = "myaddon.add_floor"
    bl_label = "Add Floor"
    bl_description = "Add boss/main floor block with file_name and collider"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):

        floor_index = find_next_numbered_name(context.scene, "floor_")
        floor_name = f"floor_{floor_index:02d}"

        # 3Dカーソルの少し下に、上面が地面になる大きめの床を作る
        floor_location = context.scene.cursor.location.copy()
        floor_location.z -= 1.0
        bpy.ops.mesh.primitive_cube_add(location=floor_location)
        floor_object = context.active_object
        floor_object.name = floor_name
        floor_object.scale = (25.0, 25.0, 1.0)

        # ゲーム側で床として読むための情報を設定する
        floor_object["file_name"] = "cube.obj"
        floor_object["collider"] = "BOX"
        floor_object["collider_center"] = mathutils.Vector((0, 0, 0))
        floor_object["collider_size"] = mathutils.Vector((2, 2, 2))
        ...
        # 現在編集中のステージへ床を入れる
        move_object_to_level_stage(floor_object, context.scene, context.scene.myaddon_edit_level_stage)

        # 床だと分かるように薄いグレーのマテリアルを付ける
        floor_material = bpy.data.materials.get("LevelFloor_Gray")
        if floor_material is None:
            floor_material = bpy.data.materials.new("LevelFloor_Gray")
        floor_material.diffuse_color = (0.55, 0.55, 0.55, 1.0)
        floor_object.data.materials.clear()
...
class MYADDON_OT_add_boss_teleport(bpy.types.Operator):
    ...
    bl_label = "Add Boss Teleport"
    bl_description = "ボスステージへ移動する入口を追加します"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):

        # 連番で BossTeleport_00 のような名前を付ける
        teleport_index = find_next_numbered_name(context.scene, "BossTeleport_")
        teleport_name = f"BossTeleport_{teleport_index:02d}"

        # 3Dカーソルの位置に、床へ置きやすい薄いキューブを作る
        teleport_location = context.scene.cursor.location.copy()
        teleport_location.z += 0.2
        bpy.ops.mesh.primitive_cube_add(location=teleport_location)
        teleport_object = context.active_object
        teleport_object.name = teleport_name
        teleport_object.scale = (1.5, 1.5, 0.2)

        # ゲーム側で読み込むためのモデル名と当たり判定を設定する
        teleport_object["file_name"] = "cube.obj"
        teleport_object["collider"] = "BOX"
        teleport_object["collider_center"] = mathutils.Vector((0, 0, 0))
        teleport_object["collider_size"] = mathutils.Vector((2, 2, 8))
        teleport_object["object_kind"] = "boss_teleport"
        # 踏んだときに読み込むボス部屋のJSONファイルを設定する
        teleport_object["target_level"] = "Resources/level/bossStage.json"
        # 現在編集中のステージへテレポーターを入れる
        move_object_to_level_stage(teleport_object, context.scene, context.scene.myaddon_edit_level_stage)

        # 見た目で入口だと分かるように水色のマテリアルを付ける
        teleport_material = bpy.data.materials.get("BossTeleport_Cyan")
        if teleport_material is None:
            teleport_material = bpy.data.materials.new("BossTeleport_Cyan")
        teleport_material.diffuse_color = (0.25, 0.9, 1.0, 1.0)
        teleport_object.data.materials.clear()
        teleport_object.data.materials.append(teleport_material)

        apply_editor_visuals(teleport_object)
        sync_level_stage_visibility(context.scene)
        sync_object_selection_filter(context.scene)
...
def requires_collider(object):
    ...
    object_kind = get_object_kind(object)
    return object_kind in {"wall", "floor", "boss_teleport"}
...
def requires_file_name(object):
    ...
    object_kind = get_object_kind(object)
    return object_kind in {"wall", "floor", "player", "boss_teleport"}
...
class MYADDON_OT_export_scene(
    ...
    stage_target: bpy.props.EnumProperty(
        name="Stage Target",
        items=[
            ("ALL", "All", "全ステージを出力します"),
            ("MAIN", "Main", "通常ステージだけ出力します"),
            ("BOSS", "Boss", "ボスステージだけ出力します"),
        ],
        default="ALL"
    )
    ...
    def parse_scene_recursive_json(self, data_parent, object, level):
        ...
        # Blender側で設定したオブジェクト種別をJSONへ出力する
        if "object_kind" in object:
            json_object["object_kind"] = object["object_kind"]

        # テレポーターが持つ移動先JSONを出力する
        if "target_level" in object:
            json_object["target_level"] = object["target_level"]
    ...
    def make_nav_mesh_json(self, export_stage="ALL"):
        ...
        for object in bpy.context.scene.objects:
            ...
            # 出力対象ステージに入っていないNavMeshは出力しない
            if not should_export_object_for_stage(object, export_stage):
                continue
    ...
    def export_json(self, export_stage="ALL"):

        """JSON形式でファイルに出力"""

        # Boss出力では空ステージにならないよう最低限の床とPlayerを保証する
        if export_stage == "BOSS":
            ensure_boss_stage_basics(bpy.context.scene)
        ...
        json_object_root["nav_mesh"] = self.make_nav_mesh_json(export_stage)
    ...
    def execute(self, context):
        ...
        # JSONファイルに出力
        self.export_json(self.stage_target)
...
# 選択中オブジェクトを現在の編集ステージへ移動する
class MYADDON_OT_move_selected_to_level_stage(bpy.types.Operator):

    bl_idname = "myaddon.move_selected_to_level_stage"
    bl_label = "Move Selected To Stage"
    bl_description = "選択中のオブジェクトを現在の編集ステージへ移動します"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):

        # 選択中のものをまとめてMainStage/BossStageへ移す
        target_stage = context.scene.myaddon_edit_level_stage
        selected_objects = list(context.selected_objects)

        if len(selected_objects) == 0:
            self.report({'WARNING'}, "移動するオブジェクトを選択してください")
            return {'CANCELLED'}

        for object in selected_objects:
            move_object_and_children_to_level_stage(object, context.scene, target_stage)

        sync_level_stage_visibility(context.scene)
        sync_object_selection_filter(context.scene)
        self.report({'INFO'}, f"選択オブジェクトを {get_level_stage_collection_name(target_stage)} へ移動しました")
        return {'FINISHED'}

# 通常ステージ/ボスステージの固定パスへJSONを出力する
class MYADDON_OT_export_level_json(bpy.types.Operator):

    bl_idname = "myaddon.export_level_json"
    bl_label = "Export Level JSON"
    bl_description = "選択したステージ用のJSONファイルへ直接出力します"

    target: bpy.props.EnumProperty(
        name="Target",
        items=[
            ("MAIN", "Main", "通常ステージJSONへ出力します"),
            ("BOSS", "Boss", "ボスステージJSONへ出力します"),
        ],
        default="MAIN"
    )
    ...
    def execute(self, context):
        ...
        # 出力先をボタン指定または現在の選択から決める
        export_target = self.target
        if export_target == "MAIN":
            filepath = context.scene.myaddon_main_level_export_path
        else:
            filepath = context.scene.myaddon_boss_level_export_path

        # 出力先が空なら処理を止める
        if filepath.strip() == "":
            self.report({'WARNING'}, "出力先JSONファイルが設定されていません")
            return {'CANCELLED'}

        # Blenderの//始まりの相対パスにも対応する
        filepath = bpy.path.abspath(filepath)

        # Export前にシーンを検証して問題があれば中止する
        issues = validate_level_scene(context.scene)
        update_validation_result(context.scene, issues)
        ...
        # 既存のExport Scene処理を使って、指定パスへ書き出す
        result = bpy.ops.myaddon.myaddon_ot_export_scene(filepath=filepath, stage_target=export_target)
        if 'CANCELLED' in result:
            return {'CANCELLED'}

        self.report({'INFO'}, f"Level JSON を出力しました: {filepath}")
        return {'FINISHED'}
...
class OBJECT_PT_file_name(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if "file_name" in context.object:
            ...
            # テレポーターの場合は移動先JSONも編集できるようにする
            if context.object.get("object_kind", "") == "boss_teleport":
                if "target_level" not in context.object:
                    context.object["target_level"] = "Resources/level/bossStage.json"
                self.layout.prop(context.object, '["target_level"]', text="Target Level")
...
class VIEW3D_PT_grid_snap(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Main/Bossの編集対象を切り替える
        stage_box = layout.box()
        stage_box.label(text="Stage Collections")
        stage_box.prop(scene, "myaddon_edit_level_stage", text="Edit")
        stage_box.operator(MYADDON_OT_move_selected_to_level_stage.bl_idname, text="Move Selected To Edit Stage")
        layout.separator()
        ...
        layout.operator(MYADDON_OT_add_floor.bl_idname, text="Add Floor")
        ...
        # コライダー線の表示を切り替える
        layout.prop(scene, "myaddon_show_stage_collider", text="Show Collider")
        ...
        # 敵の巡回経路表示を切り替える
        layout.prop(scene, "myaddon_show_stage_enemy_path", text="Show Enemy Path")
        ...
        # ステージJSON出力欄をまとめて表示する
        export_box = layout.box()
        export_box.label(text="Level JSON Export")
        export_box.prop(scene, "myaddon_level_export_target", text="Target")
        export_box.prop(scene, "myaddon_main_level_export_path", text="Main")
        export_box.prop(scene, "myaddon_boss_level_export_path", text="Boss")
        selected_export = export_box.operator(MYADDON_OT_export_level_json.bl_idname, text="Export Selected Level")
        selected_export.target = scene.myaddon_level_export_target
        main_export = export_box.operator(MYADDON_OT_export_level_json.bl_idname, text="Export Main Stage")
        main_export.target = "MAIN"
        boss_export = export_box.operator(MYADDON_OT_export_level_json.bl_idname, text="Export Boss Stage")
        boss_export.target = "BOSS"
        layout.separator()
...
class DrawCollider:
    ...
    def draw_collider():
        ...
        # 表示がOFFなら何も描画しない
        if not bpy.context.scene.myaddon_show_stage_collider:
            return
        ...
        for object in bpy.context.scene.objects:
            ...
            # 非表示状態のオブジェクトのコライダー線は描画しない
            if object.hide_get():
                continue

            # コライダーもEditで選んだステージだけ描画する
            if get_object_level_stage(object) != bpy.context.scene.myaddon_edit_level_stage:
                continue
...
class DrawEnemyPath:
    ...
    def draw_enemy_path():
        ...
        # 表示設定がOFFなら何も描画しない
        if not bpy.context.scene.myaddon_show_stage_enemy_path:
            return
        ...
        for enemy_object in bpy.context.scene.objects:
            ...
            # 非表示状態の敵の経路線は描画しない
            if enemy_object.hide_get():
                continue

            # 敵経路もEditで選んだステージだけ描画する
            if get_object_level_stage(enemy_object) != bpy.context.scene.myaddon_edit_level_stage:
                continue
            ...
            # Waypointも敵本体と同じステージのものだけ使う
            enemy_stage = get_object_level_stage(enemy_object)
            waypoint_objects = [
                waypoint_object
                for waypoint_object in waypoint_objects
                if get_object_level_stage(waypoint_object) == enemy_stage and not waypoint_object.hide_get()
            ]
...
classes = (
    ...
    MYADDON_OT_move_selected_to_level_stage,
    MYADDON_OT_export_level_json,
...
def register():
    ...
    bpy.types.Scene.myaddon_show_main_stage_objects = bpy.props.BoolProperty(
        name="Show Main Stage",
        description="MainStageコレクションの表示を切り替えます",
        default=True,
        update=on_edit_level_stage_changed
    )

    bpy.types.Scene.myaddon_show_stage_collider = bpy.props.BoolProperty(
        name="Show Stage Collider",
        description="現在のEditステージのコライダー線だけを表示します",
        default=True
    )

    bpy.types.Scene.myaddon_show_stage_enemy_path = bpy.props.BoolProperty(
        name="Show Stage Enemy Path",
        description="現在のEditステージの敵経路線だけを表示します",
        default=True
    )

    bpy.types.Scene.myaddon_show_boss_stage_objects = bpy.props.BoolProperty(
        name="Show Boss Stage",
        description="BossStageコレクションの表示を切り替えます",
        default=True,
        update=on_edit_level_stage_changed
    )
    bpy.types.Scene.myaddon_edit_level_stage = bpy.props.EnumProperty(
        name="Edit Stage",
        description="配置・編集するステージを切り替えます",
        items=[
            ("MAIN", "Main", "通常ステージを編集します"),
            ("BOSS", "Boss", "ボスステージを編集します"),
        ],
        default="MAIN",
        update=on_edit_level_stage_changed
    )
    ...
    bpy.types.Scene.myaddon_main_level_export_path = bpy.props.StringProperty(
        name="Main Level JSON",
        description="通常ステージを書き出すJSONファイル",
        default=r"C:\Users\k024g\デスクトップ\CG2\CG2_00_01\project\Resources\level\testScene.json",
        subtype="FILE_PATH"
    )

    bpy.types.Scene.myaddon_boss_level_export_path = bpy.props.StringProperty(
        name="Boss Level JSON",
        description="ボスステージを書き出すJSONファイル",
        default=r"C:\Users\k024g\デスクトップ\CG2\CG2_00_01\project\Resources\level\bossStage.json",
        subtype="FILE_PATH"
    )

    bpy.types.Scene.myaddon_level_export_target = bpy.props.EnumProperty(
        name="Export Target",
        description="固定出力ボタンで使うステージの種類",
        items=[
            ("MAIN", "Main", "通常ステージJSONへ出力します"),
            ("BOSS", "Boss", "ボスステージJSONへ出力します"),
        ],
    ...
    bpy.types.Scene.myaddon_edit_target = bpy.props.EnumProperty(
        ...
        items=[
            ("ALL", "All", "Edit all level objects"),
            ("ENEMY", "Enemy", "Edit enemies and waypoints"),
            ("WALL", "Wall", "Edit walls"),
            ("FLOOR", "Floor", "Edit floors"),
            ("NAVMESH", "NavMesh", "Edit NavMesh"),
    ...
    # アドオン有効化中はcontext.sceneが無いことがあるため、存在するときだけ初期同期する
    scene = getattr(bpy.context, "scene", None)
    if scene is not None:
        # 古い描画ハンドラを止めるため、旧表示フラグはOFFにしておく
        scene.myaddon_show_collider = False
        scene.myaddon_show_enemy_path = False

        ensure_level_stage_collections(scene)
        sync_level_stage_visibility(scene)

        # 既存オブジェクトへエディタ用の見た目と選択制限を反映する
        for object in scene.objects:
            apply_editor_visuals(object)
        sync_nav_mesh_visibility(scene)
        sync_object_selection_filter(scene)
...
def unregister():
    ...
    # 登録されている場合だけトップバーメニューから外す
    try:
        bpy.types.TOPBAR_MT_editor_menus.remove(TOPBAR_MT_my_menu.submenu)
    except Exception:
        pass

    # 描画ハンドラは存在するものだけ安全に解除する
    ...
    if DrawGrid.handle is not None:
        try:
            bpy.types.SpaceView3D.draw_handler_remove(DrawGrid.handle, "WINDOW")
        except Exception:
            pass
        DrawGrid.handle = None

    if DrawEnemyPath.handle is not None:
        try:
            bpy.types.SpaceView3D.draw_handler_remove(DrawEnemyPath.handle, "WINDOW")
        except Exception:
            pass
    ...
    # context.sceneがある場合だけ選択ロックを解除する
    scene = getattr(bpy.context, "scene", None)
    if scene is not None:
        for object in scene.objects:
            object.hide_select = False

    # クラスは登録済みのものだけ安全に解除する
    ...
    # Sceneプロパティは存在するものだけ安全に削除する
    scene_properties = [
        "myaddon_show_main_stage_objects",
        "myaddon_show_boss_stage_objects",
        "myaddon_edit_level_stage",
        "myaddon_show_grid",
        "myaddon_show_collider",
        "myaddon_show_stage_collider",
        "myaddon_show_stage_enemy_path",
        "myaddon_show_collider_gizmo",
        "myaddon_show_enemy_path",
        "myaddon_show_nav_mesh",
        "myaddon_grid_count",
        "myaddon_grid_snap_enabled",
        "myaddon_grid_size",
        "myaddon_enemy_preview_speed",
        "myaddon_main_level_export_path",
        "myaddon_boss_level_export_path",
        "myaddon_level_export_target",
        "myaddon_validation_status",
        "myaddon_validation_details",
        "myaddon_edit_target",
    ]
